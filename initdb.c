...
int initdb()
{
    ...
    else
    {
        ...
        mysql_query(&mysql, "CREATE DATABASE travel_system");//if db doesn't ,create it
...
// create all tables
    mysql_query(&mysql, "CREATE TABLE FLIGHTS(flightNum VARCHAR(100) NOT NULL PRIMARY KEY, price INT, numSeats INT, numAvail INT, FromCity VARCHAR(100), ArivCity VARCHAR(100))");
    ...
    mysql_query(&mysql, "CREATE TABLE HOTELS(name VARCHAR(100), location VARCHAR(100) NOT NULL  PRIMARY KEY, price INT, numRooms INT, numAvail INT)");
    ...
    mysql_query(&mysql, "CREATE TABLE CARS(type VARCHAR(100), location VARCHAR(100) NOT NULL  PRIMARY KEY, price INT, numCars INT, numAvail INT)");
    ...
    mysql_query(&mysql, "CREATE TABLE RESERVATIONS(custName VARCHAR(100) NOT NULL, resvType INT NOT NULL, resvKey VARCHAR(100) NOT NULL, location VARCHAR(100), FromCity VARCHAR(100), ArivCity VARCHAR(100), PRIMARY KEY (resvKey), FOREIGN KEY (custName) REFERENCES CUSTOMERS(custName))");
